...
const MODULE_RENDER_VERSION='kw184-vehicle-catalog-servis-link-644';
...
const MODULE_CALC_VERSION='kw184-vehicle-catalog-servis-link-644';
const FI={
...
const opts=(items||[]).map(it=>`<option value="${escapeHtml(it.id)}">${escapeHtml(it.partName||'(Tanpa nama)')}${it.oemCode?' — '+escapeHtml(it.oemCode):''}</option>`).join('');
...
const catalogPartId=document.getElementById('servisCatalogPartId')?document.getElementById('servisCatalogPartId').value:'';
...
const MODULE_FEATURES_VERSION='kw184-vehicle-catalog-servis-link-644';
